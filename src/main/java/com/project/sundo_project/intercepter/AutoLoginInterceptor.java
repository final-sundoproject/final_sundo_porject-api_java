package com.project.sundo_project.intercepter;

import com.project.sundo_project.entity.Company;
import com.project.sundo_project.mapper.CompanyMapper;
import com.project.sundo_project.util.LoginUtil;
...
public class AutoLoginInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (autoLoginCookie != null
            ...
            // 4. 세션아이디로 회원정보 조회
            Company foundCompany = companyMapper.findMemberBySessionId(sessionId);

            // 5. 회원이 정상조회되었고 자동로그인 만료시간 이전이라면
            //     사이트 로그인 처리(세션에 DTO세팅)를 수행
            if (foundCompany != null
                    && LocalDateTime.now().isBefore(foundCompany.getLimitTime())) {

                companyService.maintainLoginState(request.getSession(), foundCompany);
            }
        }
